<?php
    ###########################################
    function process_group_entry($entry, $edge)
    ###########################################
    {
        ...
        list($x, $entry_id) = explode("_", $entry["id"]);
        
        if( count($entry["comments"]["data"]) > 0 )
        {
            # Assignment of the first 20
            $offset = 0;
            foreach( $entry["comments"]["data"] as $this_comment )
            {
                $offset++;
                $feed_comments[] = $this_comment;
            } # end foreach
            
            # Checking if there's more
            # print_r( $entry["comments"] );
            $res = $entry["comments"];
            while( true )
            {
                # cli::write(print_r($res["paging"], true), cli::$forecolor_yellow);
                if( $res["paging"]["next"] )
                {
                    # Gonnna need to download the whole comments stream for the item
                    $continue_comments_loop = true;
                    $api_call = $entry["id"]."/comments?offset=$offset";
                    cli::write( "                    |  .----------------------------------------------------------\n", cli::$forecolor_purple );
                    cli::write( "                    |  | Entry [$entry_type:$entry_id] ".substr(str_replace("\n", "", $msg), 0, 100)."\n", cli::$forecolor_purple );
                    cli::write( "                    |  | Has comments ahead! Preparing to download them.\n", cli::$forecolor_purple );
                    cli::write( "                    |  | '--> $api_call\n", cli::$forecolor_purple );
                    try
                    {
                    ...
                    {
                        cli::write( "                    |  | Exception caught: " . $e->getMessage() . "\n", cli::$forecolor_light_purple );
                        cli::write( "                    |  | Comment download aborted.\n", cli::$forecolor_light_purple );
                        $continue_comments_loop = false;
                    } # end try...catch
                    if( $continue_comments_loop  )
                    {
                        cli::write( "                    |  | Got ".count($res["data"])." comments. Processing...\n", cli::$forecolor_purple );
                        foreach($res["data"] as $this_comment)
                        {
                            $offset++;
                            $feed_comments[] = $this_comment;
                        } # end foreach
                        cli::write( "                    |  | Done. Now comments collection has ".count($feed_comments).".\n", cli::$forecolor_purple );
                    } # end if
                    cli::write( "                    |  '----------------------------------------------------------\n", cli::$forecolor_purple );
                    # cli::write(print_r($res["paging"], true), cli::$forecolor_brown);
                }
                else
                {
                    break;
        ...
        cli::write( "                    |  | Comments:    ".count($entry["comments"]["data"])."\n", cli::$forecolor_green );
