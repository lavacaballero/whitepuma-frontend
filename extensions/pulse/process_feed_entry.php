<?php
    /**
     * Process pulse feed entry
     *
     * @param pulse_post $entry
     *
     * @returns action[]
     */
    function process_feed_entry($entry)
    {
        global $config, $feed_comments;

        $action          = new action();
        $from            = new account($entry->id_author);
        $msg             = $entry->caption . " ~ " . $entry->content;
        $entry_type      = "pulse_{$entry->type}";
        $recipient       = null;

        if( $entry_type == "rain" )
        {
            # cli::write( "                    |  .----------------------------------------------------------\n", cli::$forecolor_light_red );
            # cli::write( "                    |  | Message {$entry->id} is a rain. Entry ignored.\n", cli::$forecolor_light_red );
            # cli::write( "                    |  '----------------------------------------------------------\n", cli::$forecolor_light_red );
            $action->state = "ERROR";
            $action->info  = "RAINS_IGNORED";
            return $action;
        } # end if

        $msg = "";
        if(     ! empty($entry->caption) && empty($entry->content) )
        {
            $msg = trim($entry->caption);
        }
        elseif(   empty($entry->caption) && ! empty($entry->content) )
        {
            $msg = trim($entry->content);
        }
        elseif( ! empty($entry->caption) && ! empty($entry->content) )
        {
            $msg = trim($entry->caption) . " ~ " . trim($entry->content);
        } # end if
        ...
        $action->from_id_account  = $from->id_account;
        ...
        #===================================
        {
            ...
            if( ! empty($entry->metadata->caption_mentions) )
            {
                foreach($entry->metadata->caption_mentions as $id => $name)
                {
                    cli::write( "                    |  |              | - [$id] $name\n", cli::$forecolor_yellow );
                    $recipients[$id] = $name;
                } # end foreach
            } # end if
            if( ! empty($entry->metadata->content_mentions) )
            {
                foreach($entry->metadata->content_mentions as $id => $name)
                {
                    cli::write( "                    |  |              | - [$id] $name\n", cli::$forecolor_yellow );
                    $recipients[$id] = $name;
                } # end foreach
        ...
        {
            cli::write( "                    |  +----------------------------------------------------------\n", cli::$forecolor_light_red );
            cli::write( "                    |  | No recipients detected. Ignoring entry.\n", cli::$forecolor_light_red );
            cli::write( "                    |  '----------------------------------------------------------\n", cli::$forecolor_light_red );
            $action->state = "ERROR";
            $action->info  = "NO_RECIPIENTS_DETECTED";
            return $action;
        } # end if
