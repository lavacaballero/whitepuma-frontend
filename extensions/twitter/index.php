<?php
    /**
     * Platform Extension: Twitter index page (info)
     *
    ...
    if($admin_impersonization_in_effect)
    {
        ...
        $title_append = "Twitter TipBot instructions";
    }
    else
    {
        if( $config->user_home_shows_by_default == "multicoin_dashboard" )
...
<html xmlns:fb="http://ogp.me/ns/fb#">
    ...
    <body>
        ...
        <h2 class="ui-widget-header message_box ui-corner-all">
            How does our Twitter TipBot work?
        </h2>

        <p>Our TipBot reads the mentions stream of our <a href="https://twitter.com/<?=$config->twitter_screen_name?>" target="_blank">Twitter TipBot account</a> and looks for commands and recipient mentions, then
        issues the transfer from the sender's to the recipients wallets <b>without charging any fee for the time being</b>.</p>
        ...
        <ul>
            <li>
                <u>New <?=$config->app_single_word_name?> users coming from Twitter</u>:
                You already have your account. You just need to friend <a href="https://twitter.com/<?=$config->twitter_screen_name?>"
                target="_blank">@<?=$config->twitter_screen_name?></a>, go to twitter and tip using any of the syntaxes described below.
            </li>
            <li>
                <u>Existing <?=$config->app_single_word_name?> users without connection to our Twitter app</u>:
                You just need to <a href="#connect">connect your Twitter account</a> with <?=$config->app_single_word_name?>!
            </li>
            <li>
                <u>Existing Facebook Tipping Apps users</u> (Doge/Multicoin):
                you may already have tied your balances with <?=$config->app_single_word_name?>,
                so you can go <a href="#connect">straight to the bottom</a> of this page to <b>connect your Twitter account</b>.
            </li>
        </ul>

        <div class="ui-state-highlight message_box ui-corner-all">
            <span class="fa fa-warning"></span><b>Important!</b>
            Our <a href="https://twitter.com/<?=$config->twitter_screen_name?>" target="_blank">TipBot</a>
            <b>only listens to tip commands.</b> You can't use it to <u>ask for your balances</u>, <u>withdraw</u>,
            get help, see your history or any other thing <u>but tipping</u>.<br>
            <b>Make sure you follow it</b> or you will <u>never receive notifications</u> with your updated balances
            when your tips are delivered!
        </div>

        <p><b>Short story</b>: make sure you <u>have your Twitter account connected here</u>
        and <a href="https://twitter.com/<?=$config->twitter_screen_name?>" target="_blank"><b>you follow our TipBot</b></a>
        and you'll be set!</p>

        <h2>Extra info about our TipBot account</h2>

        <ul>
            <li>As stated above, it <u>only listens to tip commands</u>. Nothing else.</li>
            <li>The only things it tweets are <u>invitations to unregistered users</u>. Once a user has approved
            our app, all communication is sent through <u>direct messages</u>.</li>
            <li>We don't send news or contact users or send direct messages other than tip delivery notifications
            through this account. All news are send through the <?=$config->twitter_news_account_link?> account.</li>
            <li>Once you follow it, you may prefer to mute it unless you want to know when an invitation is sent to an
        ...
        an account is being created for them and your tweet
        is replied, informing them -and you- about their new account with a link to our
        <a href="<?=$config->twitter_about_page?>" target="_blank">Tweeter TipBot info page</a>. Once they
        authorize our App they'll be able to claim their tips.</p>

        <h2>Got a new account generated automatically? <u>Re-route it!</u></h2>

        <p>If you already are a <?=$config->app_single_word_name?> user, you haven't tied your Twitter account
        and you got a welcome message from our TipBot, you may want to re-route your tips from your new Twitter account
        ...
        <p>There's one disadvantage: you will need to manually send existing coins, and since your Twitter account
        is already tied to a new <?=$config->app_single_word_name?> account, you'll not be able to use Twitter to sign into
